...
param_grid = {
    ...
    'max_depth': [10, 20, 30, 40, 50, None], # 最大深度
    'min_samples_split': [2, 5, 10], # 最小样本分割数
    'min_samples_leaf': [1, 2, 4], # 最小样本叶片数
...
grid_search = GridSearchCV(rf, param_grid, scoring='accuracy', cv=cv, n_jobs=-1, verbose=1)
...
# 使用最佳参数训练随机森林分类器并统计训练时间
start_time = time.time()
best_rf = RandomForestClassifier(**best_params)
best_rf.fit(scaled_data, ALL_label)
rf_training_time = time.time() - start_time
print(f"RF_3 training time: {rf_training_time} seconds")

#使用最佳参数重新训练随机森林分类器
best_rf = RandomForestClassifier(**best_params)
best_rf.fit(scaled_data, ALL_label)

# best_rf的路径，best_rf_3_特征数量_最佳准确率
best_rf_path = "model/best_rf_3_" + str(feature_num) + '_' + str(round(best_accuracy, 3)*100) + ".pkl"
